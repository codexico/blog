# Francisco Eugenio Endler Kahil

## Desenvolvedor Full Stack

## Contato

São Paulo, Brasil

[+55 11 987960288](tel:+5511987960288)

[francisco.kahil@gmail.com](mailto:francisco.kahil@gmail.com)

[LinkedIn](https://www.linkedin.com/in/franciscokahil/), [github](https://github.com/codexico/), [codexico.com.br](https://codexico.com.br/blog/)

## Resumo

Desenvolvedor Full Stack há mais de 15 anos desenvolvendo aplicações web escaláveis e acessíveis. Especializado em front-end com um forte foco em JavaScript, TypeScript e React.

Com experiência em diversos setores, especialmente e-commerce e marketing, e uma variedade de tecnologias e metodologias, possuo uma compreensão única de requisitos de projetos e de dinâmicas de equipes.

Recentemente, realizei uma pausa intencional na carreira para me dedicar a questões de saúde familiar e pessoal. Mantive-me tecnicamente ativo através de estudos e experimentos com tecnologias modernas.

Agora, com foco renovado, busco novas oportunidades para aplicar minha senioridade e visão estratégica em projetos inovadores.

## Experiência

### - Projetos recentes

2026

- [Meus Remedinhos](https://play.google.com/store/apps/details?id=com.franciscokahil.appMeusRemedinhos) - Android App feito em Kotlin
- [No Posts No Shorts For YouTube](https://github.com/codexico/no_posts_no_shorts) - Extensão de navegador para [Firefox e Firefox for Android](https://addons.mozilla.org/en-US/firefox/addon/no-posts-no-shorts-for-youtube/), também para [Google Chrome](https://chromewebstore.google.com/detail/no-posts-no-shorts-for-yo/gnlkanbjjhfpfbgbkhcfhnggknipokde?pli=1)
- [utf-8 chars](https://chromewebstore.google.com/detail/utf-8-and-unicode-charact/fcemphgmjnjpmmdhcedhjiegickfbiia?pli=1) - Extensões com 9k usuários para [Chrome](https://github.com/codexico/UTF-8-Chrome-Extension) e [Firefox](https://github.com/codexico/UTF-8-Firefox-Extension) para listar e usar caracteres e emojis (2013 - 2026).

### - [EITA](https://eita.coop.br/) - Desenvolvedor Android React Native

janeiro de 2023 - julho de 2023 (7 meses)

- Desenvolvi uma aplicação React Native com geolocalização, bancos de dados NoSQL offline assíncronos, sincronização entre celulares sem internet, Material UI e animações.

### - [Pipefy](https://www.pipefy.com/pt-br/) - Desenvolvedor Web Full Stack

dezembro de 2020 - abril de 2022 (1 ano e 5 meses)

- Ao colaborar com equipes multifuncionais, fui fundamental na entrega de três projetos críticos em dificuldades, incluindo nova integração com backend, novo layout do app, desenvolvimento de design system, remoção de bugs antigos e correções de falhas de desempenho.
- Responsável pelo desenvolvimento e implementação de integração de e-mail à plataforma com interface semelhante a webmail.
- Comunicação diária em inglês.

### - [Magazine Luiza (Luizalabs)](https://www.magazineluiza.com.br/) - Desenvolvedor Web Full Stack

dezembro de 2016 - dezembro de 2020 (4 anos e 1 mês)

- Na equipe de checkout do e-commerce, implementei novos métodos de pagamento, login, segurança, CI/CD e analytics.
- Responsável por separar do back e front, aumentando a estabilidade, o desempenho e a testabilidade.
- Unifiquei os aplicativos móveis e desktop em um único aplicativo responsivo, criando uma melhor integração visual e velocidade para implementar novos recursos.

### - Titans Group - Desenvolvedor Web Full Stack

novembro de 2014 - maio de 2016 (1 ano e 7 meses)

- Refatoração de plataforma white label com milhões de usuários na América Latina, com diversas marcas e temas, web e mobile, reduzindo em 80% a base de código.
- Desenvolvimento do sistema em JavaScript puro para celulares de baixa performance.
- Responsável por otimização e implementação de ferramentas de automatização, ci/cd, testes e boas práticas.
- Como resultado a introdução de novos clientes passou de semanas para dias.

### - Pivot Produtora Interativa - Desenvolvedor Web Full Stack

maio de 2012 - outubro de 2014 (2 anos 6 meses)

- Desenvolvimento de websites trabalhando para diversas agências de publicidade (Riot, Y&R, ID, Wunderman, Almap, Digitas ...) e suas diversas metodologias e requisitos, foco na agilidade e layouts no nível de pixel perfect.
- Desenvolvimento para grandes marcas, de portais, campanhas, sistemas internos e aplicativos para facebook, tanto web quanto tablet e mobile.
- Criação e padronização de plugins jQuery.
- Criação de framework js simples para agilidade da equipe no bootstrap de novos projetos, passando de horas para minutos.

### - Fracta - Desenvolvedor Web Full Stack

janeiro de 2011 - janeiro de 2012 (1 ano)

- Desenvolvimento de websites em geral, do photoshop ao servidor.
- Desenvolvimento de módulos para Drupal.

### - Dennova - Desenvolvedor Web Full Stack

julho de 2008 - novembro de 2009 (1 ano 5 meses)

- Construção de CMS em PHP, especialmente módulo de indexação e busca.
- Desenvolvimento de websites em geral.
- Criação de temas e módulos em Drupal.

### - Freelancer e projetos pessoais - Desenvolvedor Web Full Stack

**2009 - 2026**

- [No Posts No Shorts For YouTube](https://github.com/codexico/no_posts_no_shorts) - Brwoser Extension to remove Shorts and Posts from YouTube, versions for Chrome, Firefox, Opera and Firefox for Android (2026).

- [Local AI for Kids](https://github.com/codexico/ai-kids) - IA pronta para crianças no Steam Deck usando Ollama e Open WebUI (2026).

- [Barking Gecko](http://www.barkinggecko.com.au) - Website australiano em Drupal (2009).

- [GEIO](https://github.com/codexico/geio) - Sistema de controle de campanhas promocionais para Shopping
  Centers em CakePHP (2010).

- [jquery-plugin-lancelot-autolink](https://github.com/codexico/jquery-plugin-lancelot-autolink) - navegação de links por hover (2010).

- [Vogon](https://github.com/codexico/vogon) - Sistema de acompanhamento de variação de preços em diversas lojas. Ruby on Rails, JavaScript (2011).

- [ttlocal](https://github.com/codexico/ttlocal) - Todos os Trending Topics do Twitter, Youtube e outras fontes em uma só página. Uso de Twitter API, Youtube API, PHP, jQuery, CSS (2012).

- [jquery-validation](https://github.com/codexico/jquery-validation) - correção de bug na validação de horário de verão (2013).

- [utf-8 chars](https://chromewebstore.google.com/detail/utf-8-and-unicode-charact/fcemphgmjnjpmmdhcedhjiegickfbiia?pli=1) - Extensões com 30k usuários para [Chrome](https://github.com/codexico/UTF-8-Chrome-Extension) e [Firefox](https://github.com/codexico/UTF-8-Firefox-Extension) para listar e usar caracteres e emojis (2013 - 2025).

- [Ligeirinho](https://github.com/codexico/ligeirinho) - Extensão para Google Chrome, sistema de procura de produtos e comparação de preços baseado em API do Buscapé (2014).

- [Rspec::Visual](https://github.com/codexico/rspec-visual) - Ruby Gem para testes através de comparação de screenshots (2015).

- [changelogkeeper](https://github.com/codexico/changelogkeeper) - NPM package para escrever changelogs automaticamente (2019).

## Formação acadêmica


### - [MBA em Desenvolvimento de Aplicações e Games para Dispositivos Móveis e Internet das Coisas](https://web.archive.org/web/20150423035507/http://www.fiap.com.br/mba/mba-em-desenvolvimento-de-aplicacoes-e-games-para-dispositivos-moveis-internet-das-coisas/)
outubro de 2015 - outubro de 2016 (12 meses - 360h)

Faculdade de Informática e Administração Paulista - [FIAP](https://www.fiap.com.br/)

### - [Tecnologia em Análise e Desenvolvimento de Sistemas](https://www.fiap.com.br/graduacao/tecnologo/analise-e-desenvolvimento-de-sistemas/)
outubro de 2008 - outubro de 2009 (24 meses - 2000h)

Faculdade de Informática e Administração Paulista - [FIAP](https://www.fiap.com.br/)

### - Física
1996 - 1999 (90% concluído)

Universidade Estadual de Campinas - [UNICAMP](https://unicamp.br/)

## Cursos

#### Formação Ruby on Rails​

2010 (52h) - [Caelum​](http://www.caelum.com.br/cursos/rails/)

#### Academia do Agile

2010 (108h) - [Globalcode](http://www.globalcode.com.br/treinamentos/carreiras/academia-do-agile)

#### Especialização em Modelagem Visual UML 2.0​

2009 - Faculdade de Informática e Administração Paulista - FIAP

#### Especialização em Servidores Linux

2008 (20h) - Guia do Hardware

#### Manutenção, Montagem e Configuração de Microcomputadores

2001 - Senai Armando Arruda Pereira

## Competências

### - Front-End:

- JavaScript, TypeScript React, Redux, CSS, HTML
- Desenvolvimento mobile, responsivo, Android
- UI/UX, Design system, acessibilidade
- Testes automatizados, visuais, Jest, Webpack

### - Back-End:

- CI/CD, Git, testes automatizados, Docker, AWS, GCP
- Python, Node.js, Ruby on Rails
- SQL, NoSQL, GraphQL

### - Soft Skills:

- Experiência de mais de 15 anos na área
- Metodologias Ágeis
- Trabalho em equipe
- Adaptabilidade
- Liderança

## Idiomas

### - Português
Nativo

### - Inglês
Avançado (B2) - 1 ano e meio trabalhando em inglês na Pipefy
